...
from SQLBDD import GenericCrud
from PokeballClass import Pokeball
...
_pokeball = Pokeball()
# pokeball = Pokeball()
pokedollars = _inventory.GetMoney()
ballInTable = _genericCrud.GetAll("pokeball")
ball = []
for element in ballInTable:
    ball.append( {
        "id" : element[0],
        "name" : element[1],
        "catchRate" : element[2],
...
def shop(pokeball):
    ...
    chosenPokeball = int(input("Séléctionner une PokeBall à acheter : 1-Pokeball " + str(pokeball[0]["price"]) + "$  2-Superball " + str(pokeball[1]["price"]) + "$  3-Hyperball " + str(pokeball[2]["price"]) +"$  4-Materball " + str(pokeball[0]["price"]) +"$ "))
    if chosenPokeball == 1 :
        addBallToInventory(pokeball[0]["price"], pokeball[chosenPokeball-1])
    elif chosenPokeball == 2 :
        addBallToInventory(pokeball[1]["price"], pokeball[chosenPokeball-1])
    elif chosenPokeball == 3 :
        addBallToInventory(pokeball[2]["price"], pokeball[chosenPokeball-1])
    elif chosenPokeball == 4 :
        addBallToInventory (pokeball[3]["price"], pokeball[chosenPokeball-1])
    else :
...
def addBallToInventory(price, pokeball) :
    ...
    if quantity*price > pokedollars:
        ...
        _pokeball.UpdateNbOfPokeBall(pokeball["name"], pokeball["quantity"])
        time.sleep(4)
        mainMenu()
...
def showInventory() :
    ...
    for element in ball:
        pokeBallInfosToShow += element["name"] + " : " + str(element["quantity"]) + "   "
    for element in pokemonInventory :
